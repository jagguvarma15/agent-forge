...
def test_assemble_no_references(mock_deployments_path: Path) -> None:
    ...
    out = assemble(recipe, language="python", framework="none", deployments_path=mock_deployments_path)
...
def test_assemble_relative_links(mock_deployments_path: Path) -> None:
    ...
    out = assemble(recipe, language="python", framework="langgraph", deployments_path=mock_deployments_path)
...
def test_assemble_alias_resolution(mock_deployments_path: Path) -> None:
    ...
    out = assemble(recipe, language="python", framework="pydantic_ai", deployments_path=mock_deployments_path)
    rel_paths = {p.name for p in out.referenced_paths}
...
def test_assemble_filters_wrong_language_framework(mock_deployments_path: Path) -> None:
    recipe = _recipe(mock_deployments_path, "docs-rag-qa")
    out = assemble(recipe, language="typescript", framework="vercel_ai_sdk", deployments_path=mock_deployments_path)
    rel_paths = {p.name for p in out.referenced_paths}
...
def test_assemble_skips_missing_reference(
    mock_deployments_path: Path, capsys
) -> None:
...
def test_token_estimate_monotonic(mock_deployments_path: Path) -> None:
    ...
    short_ctx = assemble(short, language="python", framework="none", deployments_path=mock_deployments_path)
    long_ctx = assemble(long, language="python", framework="langgraph", deployments_path=mock_deployments_path)
